...
def simulate_innings(batting_team, bowling_team, balls=100, target=None, print_over_summary=False):
	...
	for ball_no in range(balls):
		...
		# end of over - print summary and swap strike (no swap in last mode since only one batsman)
		if (ball_no + 1) % 5 == 0:
	...
	# After the loop ends, if we haven't printed the last over summary (because the innings ended normally, not due to target or all-out), print it now
	if print_over_summary and balls_bowled > 0:
		# check if the last ball wasn't at an over boundary
		last_ball_index = balls_bowled - 1
		if (last_ball_index + 1) % 5 != 0:
			# we have a partial over that wasn't printed
			over_num = last_ball_index // 5 + 1
			b = bowlers_stats[over_bowler['player_id']]
			overs_done = b['balls'] // 5
			maidens = b.get('maidens', 0)
			print(f"Over {over_num}: {total_runs}/{total_wickets}")
			print(f"Bowler: {b['name']} {overs_done}.{b['balls'] % 5}-{maidens}-{b['runs']}-{b['wickets']}")
			# batters currently in
			batters_line = []
			if striker_idx is not None:
				s = batsmen_stats[batting_team[striker_idx]['player_id']]
...
def choose_team_from_list(players, prompt="Choose a team"):
	"""Prompt user to select a team from available saved teams or create new."""
	...
	for i, fname in enumerate(team_files, start=1):
		# try to load and show team name
		teams_dir = os.path.join(os.path.dirname(__file__), 'json', 'teams')
		path = os.path.join(teams_dir, fname)
		try:
			with open(path, encoding='utf-8') as f:
				team_data = json.load(f)
				team_name = team_data.get('team_name', fname.replace('.json', ''))
				print(f"  {i}. {team_name}")
		except Exception:
			print(f"  {i}. {fname.replace('.json', '')}")
	...
	while True:
		...
		try:
			...
			if idx == len(team_files) + 1:
				# random team
				return pick_computer_team(players, exclude_ids), "Random Team"
...
def main():
	...
	if args.demo:
		...
		# Prompt user to select their team from saved teams
